...
Judging from the above output $\mathcal{M}_1$ seems to be the preferable model, since both the $\textit{DIC}$ and $\textit{WAIC}$ agree.

```{r}
BF <- function(Y1, Y2, c) {
  numer <- factorial(Y1 + 1) * factorial(Y2 + 1) * 2^(Y1 + Y2 + 1) 
  denom <- factorial(Y1 + Y2 + 1)
}
```

We encounter a problem evaluating this numerically...

## Exercise 3

Njaaaaaawwrrrrrrrrrrrrrrrrrrrrrr
