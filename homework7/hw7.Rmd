...
mcmc_october <- function(model_no, c, params) {
  ...
  samples <- coda.samples(model_october, variable.names = c("like", "lambda"),
                          n.iter = 2e+4, progress.bar = "none")
  
  likes <- 2:3; lambdas <- 1
  if (model_no == 1) { likes <- 3:4; lambdas <- 1:2 }
    
  like  <- rbind(samples[[1]][,likes], samples[[2]][,likes])
  L_bar <- colMeans(like)
  ...
  if (model_no == 2)
  return(append(samples[[1]][,lambdas], samples[[2]][,lambdas]))
}
```
...
$$We can obtain an approximate estimate by taking the posterior mean from the MCMC chain returned by the \`mcmc_october\` function.

```{r}
lambda12 <- mcmc_october(1, 1)
lambda0  <- mcmc_october(2, 1)
```
