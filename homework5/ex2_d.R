...
model_pred_uninf <- jags.model(model_init_pred_uninf, data = data, n.chains = 2)

update(model_pred_uninf, 1e+4)

params <- c("beta", "Y_pr")

samples_pred_uninf <- coda.samples(model_pred_uninf, variable.names = params, n.iter = 2e+4)
...
model_pred_lasso <- jags.model(model_init_pred_lasso, data = data, n.chains = 2)

update(model_pred_lasso, 1e+4)
...
for (i in 1:6) {
  ...
  PPD_lasso <- samples_pred_lasso[[1]][,i]

  plot(density(PPD_uninf))
  lines(density(PPD_lasso), lty = 2)
